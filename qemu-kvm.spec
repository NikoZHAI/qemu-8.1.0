...
%global modprobe_kvm_conf rh-extra/kvm.conf
%ifarch s390x
    %global modprobe_kvm_conf rh-extra/kvm-s390x.conf
%endif
%ifarch %{ix86} x86_64
    %global modprobe_kvm_conf rh-extra/kvm-x86.conf
%endif
...
Source0: qemu-%{version}%{?rcstr}.tar.xz
...
install -D -m 0644 rh-extra/qemu-guest-agent.service %{buildroot}%{_unitdir}/qemu-guest-agent.service
install -D -m 0644 rh-extra/qemu-ga.sysconfig %{buildroot}%{_sysconfdir}/sysconfig/qemu-ga
install -D -m 0644 rh-extra/99-qemu-guest-agent.rules %{buildroot}%{_udevrulesdir}/99-qemu-guest-agent.rules
...
install -D -p -m 0644 rh-extra/vhost.conf %{buildroot}%{_sysconfdir}/modprobe.d/vhost.conf
...
install -p -m 0644 rh-extra/README.tests %{buildroot}%{testsdir}/README
...
%if %{have_modules_load}
    install -D -p -m 644 rh-extra/modules-load.conf %{buildroot}%{_sysconfdir}/modules-load.d/kvm.conf
%endif

%if %{have_memlock_limits}
    install -D -p -m 644 rh-extra/95-kvm-memlock.conf %{buildroot}%{_sysconfdir}/security/limits.d/95-kvm-memlock.conf
%endif

# Install rules to use the bridge helper with libvirt's virbr0
install -D -m 0644 rh-extra/bridge.conf %{buildroot}%{_sysconfdir}/%{name}/bridge.conf
